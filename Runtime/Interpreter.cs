...
namespace ScriptStack.Runtime
{
    ...
    public class Interpreter
    {
        ...
        private void Arithmetic()
        {
            ...
            // Special rules for additions of strings and arrays
            if (instruction.OpCode == OpCode.ADD)
            {
                ...
                if (typeDest == typeof(string))
                {
                    ...
                    if(typeSource == typeof(char))
                        Assignment(instruction.First, dst.ToString() + (char)src);

                    else
                        Assignment(instruction.First, dst.ToString() + src.ToString());
                ...
                if (typeDest == typeof(ArrayList))
                {
                    ...
                    tmp.Add((ArrayList)dst);

                    tmp.Add(src);

                    Assignment(instruction.First, tmp);
            ...
            // Special rules for substraction of strings and arrays
            if (instruction.OpCode == OpCode.SUB)
            {
                ...
                if (typeDest == typeof(string))
                {
                    ...
                    Assignment(instruction.First, dst.ToString().Replace(src.ToString(), ""));
            ...
            // currently everything is converted to float.. no double
            double dstVal = 0.0;
            ...
            if (typeDest == typeof(int))
                ...
                throw new ExecutionException("Der Typ '" + typeDest.Name + "' kann in arithmetischen Operationen als Ziel (links des Operators) nicht verarbeitet werden.");
            ...
            if (typeSource == typeof(int))
                ...
                throw new ExecutionException("Der Typ '" + typeSource.Name + "' kann in arithmetischen Operationen als Quelle (rechts des Operators) nicht verarbeitet werden.");

            switch (instruction.OpCode)
            {

                case OpCode.ADD: result = dstVal + srcVal; break;
                case OpCode.SUB: result = dstVal - srcVal; break;
                case OpCode.MUL: result = dstVal * srcVal; break;
                case OpCode.DIV: result = dstVal / srcVal; break;
                case OpCode.MOD: result = dstVal % srcVal; break;

                default:
                    throw new ExecutionException("Die Anweisung '" + instruction.OpCode + "' kann in einer arithmetischen Operation nicht verarbeitet werden.");
            ...
            {
                // if dst was lower than src, its a fractal. convert to float
                if(dstVal < srcVal)
                    Assignment(instruction.First, (float)result);
                else
                    Assignment(instruction.First, (int)result);
            }

            // If one was a double return a double
            else if (typeDest == typeof(double) || typeSource == typeof(double))
                Assignment(instruction.First, (double)result);

            // otherwise.., If one was a float return a float
