    ...
    public class Parser
    {
        ...
        private Variable RoutineCall()
        {
            ...
            if (routine.ParameterTypes.Count > parameterCount)
                throw new ParserException("Der Aufruf der Routine '" + identifier + "' hat fehlende Parameter.");

            if (routine.ParameterTypes.Count < parameterCount)
                throw new ParserException("Der Aufruf der Routine '" + identifier + "' hat zu viele Parameter.");
        ...
        private Variable FunctionCall(bool background)
        {
            ...
            if (forwardDeclared)
            {
                ...
                if (function.ParameterCount > parameterCount)
                    throw new ParserException("Der Aufruf der Routine '" + name + "' hat fehlende Parameter.");

                // \todo should we just throw the rest?
                if (function.ParameterCount < parameterCount)
                    throw new ParserException("Der Aufruf der Funktion '" + name + "' hat zu viele Parameter.");
        ...
        private void Statement()
        {
            ...
            switch (token.Type)
            {
                ...
                default: throw new ParserException("Ein in einem Statement unerwarteter Token '" + token.Lexeme + "' wurde gefunden.", token);
        ...
        private void LockedStatementList()
        {
            ...
            if (token.Type != TokenType.Lock)
                throw new ExecutionException("Keyword 'lock' erwartet.");
        ...
        private void Yield()
        {
            ...
            if (token.Type != TokenType.Yield)
                throw new ParserException("Keyword 'yield' erwartet.", token);
